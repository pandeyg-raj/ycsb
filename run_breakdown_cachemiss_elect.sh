...
stop_cluster() {
    ...
    for node in "${BD_NODES[@]}"; do
        ...
        ssh ${SSH_USER}@${ip} \
            "ps -ef | grep '[j]ava' | grep -iE 'cassandra|elect' | awk '{print \$2}' | xargs kill 2>/dev/null; true"
        local attempts=0
        while ssh ${SSH_USER}@${ip} \
            "ps -ef | grep '[j]ava' | grep -iE 'cassandra|elect' > /dev/null 2>&1"; do
            sleep 10; attempts=$((attempts + 1))
            if [ "$attempts" -ge 6 ]; then
                ssh ${SSH_USER}@${ip} \
                    "ps -ef | grep '[j]ava' | grep -iE 'cassandra|elect' | awk '{print \$2}' | xargs kill -9 2>/dev/null; true"
                sleep 5; break
            fi
        done
        echo "  ${ip} stopped"
...
hard_restart_cluster() {
    ...
    for node in "${BD_NODES[@]}"; do
        local ip="10.10.1.$node"
        ssh ${SSH_USER}@${ip} "cd ${CASS_DIR} && ${CASS_BIN} > /dev/null 2>&1"
        local a=0
        until ssh ${SSH_USER}@${ip} \
            "${CASS_DIR}/bin/nodetool status 2>/dev/null | grep '${ip}' | grep -q 'UN'"; do
...
echo "Is this EC, REP, or ELECT?"; read EXP_LABEL
read -p "Write (load) threads: " WTHREADS
read -p "Run threads (measure): " THREADS

if echo "$EXP_LABEL" | grep -qi "elect"; then
    SYS_KIND="elect"
    CASS_DIR="/mydata/elect/src/elect"                        # ELECT home; binary at bin/elect
    CASS_BIN="bin/elect"
    CREATE_TABLE_BIN="create_table_rep_compr_${COMPRESSION}"  # ELECT table == stock Cassandra table (CONFIRM)
    BREAKDOWN_ENABLED=0                                        # 'nodetool breakdown' is a LEAST-only patch; ELECT lacks it
elif echo "$EXP_LABEL" | grep -qi "rep"; then
    SYS_KIND="rep"
    CASS_DIR="/mydata/cassandra"
    CASS_BIN="bin/cassandra"
...
if [ "$SYS_KIND" = "ec" ]; then shard_size=$(( FIELD_LENGTH / 3 )); else shard_size=$FIELD_LENGTH; fi   # elect/rep: full object
...
if [ "$DISABLE_COMPACTION_DURING_MEASURE" = "1" ]; then
    ...
    if [ "$BREAKDOWN_ENABLED" = "1" ]; then
        echo "--- Reset breakdown (BEFORE) ---"
        for node in "${BD_NODES[@]}"; do
    ...
    echo "--- Baseline snapshots/tablestats/compaction (BEFORE 50/50) ---"
    if [ "$BREAKDOWN_ENABLED" = "1" ]; then
        for node in "${BD_NODES[@]}"; do
            ssh ${SSH_USER}@10.10.1.$node "${CASS_DIR}/bin/nodetool breakdown --reset"
        done
