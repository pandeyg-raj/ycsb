...
for i in "${!WORKLOAD_LABELS[@]}"; do
  ...
  for i in $(seq 1 $REPEAT); do
    OutputFile="${OUT_DIR}/${EXP_LABEL}_run${FIELD_LENGTH}Bytes.log" 
    TRACE_FLAGS=""
    if [ "$i" -eq 5 ]; then
      TRACE_FLAGS="-p cassandra.tracing=true"
      MEASURE_OPS=21000
      OutputFile="${OUT_DIR}/${EXP_LABEL}_${workload}_Trace${FIELD_LENGTH}Bytes.log" 
    fi
